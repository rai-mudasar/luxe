...
export default function FeaturedCards() {
    const featuredCards = [
        ...
        {name: "Sale", href: "/sale", bgColor: "bg-[#694A3E] hover:bg-[#7E4D3D] md:col-span-2 lg:col-span-1"},
    ]
  return (
    <div className='w-full bg-[#EBE9E8] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center gap-3 md:gap-5 pt-7 md:pt-20 px-7 md:px-17'>
        {
            featuredCards.map(card => (
                <Link href={card.href} className={`w-full h-50 md:h-80 ${card.bgColor} rounded-xl flex items-end pb-9 pl-4 transition-all delay-10 duration-300 hover:scale-[1.04] hover:cursor-pointer`} key={card.name}>
